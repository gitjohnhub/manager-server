...
router.get('/all', async (ctx) => {
  ...
  log4js.info('get account success');
  const {page,skipIndex} = util.pager(ctx.request.query)
  let params = {}
  // if (userId) params.userId = userId;
  // if(state && state != 0) params.state = state;
  try {
    const query = account.find(params)
    const list = await query.skip(skipIndex).limit(page.pageSize)
    const total = await account.countDocuments(params)
...
router.post('/add', async (ctx) => {
  log4js.info('add account success');
  try {
    const {userName} = ctx.request.body
    log4js.debug(userName);

    const item = await new account(ctx.request.body);
    await item
      .save()
      .catch((err) => {
        log4js.info(err);
...
router.get('/stat_by_dept',async (ctx)=>{
  log4js.info('get  account statistics success')
