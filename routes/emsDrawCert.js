...
const emsDrawCert = require('../models/emsDrawCertSchema');
const util = require('../utils/util');
const log4js = require('../utils/log4j');

router.prefix('/emsDrawCert');

router.get('/all', async (ctx) => {
  // const {dept,startDate,endDate}  = ctx.request.query
  console.log(ctx.request.query)
  log4js.info('get emsDrawCert success');
  ...
  try {
    const query = emsDrawCert.find(params).sort({ createTime: -1 });
    const list = await query.skip(skipIndex).limit(page.pageSize)
    const total = await emsDrawCert.countDocuments(params)
...
router.post('/add', async (ctx) => {
  log4js.info('add emsDrawCert success');
  try {
    const item = await new emsDrawCert(ctx.request.body);
    await item
      .save()
      .catch((err) => {
        log4js.info(err);
...
router.post('/update', async (ctx) => {
  try {
    ...
    const user = await emsDrawCert.findById(_id)
    log4js.info(user)
    user.dept = dept
    user.item = item
    user.result = result
...
router.get('/stat_by_dept',async (ctx)=>{
  log4js.info('get  emsDrawCert statistics success')
  const {startDate,endDate}= ctx.request.query
  if(startDate){
    console.log('startDate=>',new Date(startDate))
    try {
      let data = await emsDrawCert.aggregate([
        { $match: { createTime: {
